...
/** @file
@brief Subscribe to received Radio messages.
@ingroup ex
*/
                         ...
                         void* ctx)
{
    if((NULL == notif) || (NULL == msg.data) || (0UL == msg.length) || (NULL == ctx)) {
        ...
        printf("Timestamp: %llu [ms]\n", (unsigned long long)notif->radio_params.timestamp);
        ...
        printf("Source address: %02X:%02X:%02X:%02X:%02X:%02X\n",
               notif->radio_params.source_address[0],
               notif->radio_params.source_address[1],
               notif->radio_params.source_address[2],
               notif->radio_params.source_address[3],
               notif->radio_params.source_address[4],
               notif->radio_params.source_address[5]);
        printf("Interface ID: %lu\n", (unsigned long)notif->radio_params.interface_id);
        printf("RSSI: %d [dBm]\n", notif->radio_params.rssi);
...
int main(int argc, char* argv[])
{
    ...
    error = error || cms_rio_rx_subscribe(&session,
                                          FILTER_PACKET,
                                          &rio_notif_cb,
                                          &filtered_ctx,
                                          &filtered_subs_id);
    ...
    if(error) {
        printf("Unable to subscribe to radio rx notifications for packet type: %u\n", FILTER_PACKET);
    }
    ...
    error = error || cms_rio_rx_subscribe(&session,
                                          ...
                                          &all_packet_subs_id);
    if(error) {
        printf("Unable to subscribe to all radio rx notifications\n");
    } else {

        /* Wait some messages on the wildcard subscription */
        static const uint32_t EXIT_ON_RECV_COUNT = 50UL;
        while(all_packet_ctx.cnt < EXIT_ON_RECV_COUNT) {
            sleep(1);
        }
    }

    /* Unsubscribe */
    error = error || cms_rio_rx_unsubscribe(&session, filtered_subs_id);
    error = error || cms_rio_rx_unsubscribe(&session, all_packet_subs_id);
